    ...
    public class Ch_ShootAction : _Action
    {
        ...
        { 
            ...
            if (controller.m_CharacterController.playerArm.transform.GetChild(0).childCount == 1)
            {
                ...
                if (controller.m_CharacterController.m_ControlConfig.moveArmWithRightStick)
                {
                    Debug.Log("right");
                    controller.m_CharacterController.JoyRotation(controller.m_CharacterController.m_ControlConfig.RightHorizontal, controller.m_CharacterController.m_ControlConfig.RightVertical);
                }
                else if (!controller.m_CharacterController.m_ControlConfig.moveArmWithRightStick)
                {
                    Debug.Log("left");
                    controller.m_CharacterController.JoyRotation(controller.m_CharacterController.m_ControlConfig.LeftHorizontal, controller.m_CharacterController.m_ControlConfig.LeftVertical);
                }
                   
                    // Shoot condition
                if (controller.m_CharacterController.currentWeapon.autoFire == false)
                {
                    if (Input.GetButtonDown(controller.m_CharacterController.m_ControlConfig.shootInput.ToString()) && controller.m_CharacterController.currentWeapon.isGrabbed)
                    {
                ...
                {
                    if (Input.GetButton(controller.m_CharacterController.m_ControlConfig.shootInput.ToString()) && controller.m_CharacterController.currentWeapon.isGrabbed)
                    {
                        controller.m_CharacterController.currentWeapon.Shoot();
                ...
                if (controller.m_CharacterController.m_ControlConfig.moveArmWithRightStick)
                {
                    controller.m_CharacterController.armRotation(controller.m_CharacterController.m_ControlConfig.RightHorizontal, controller.m_CharacterController.m_ControlConfig.RightVertical);
                }
                else if (!controller.m_CharacterController.m_ControlConfig.moveArmWithRightStick)
                {
                    controller.m_CharacterController.armRotation(controller.m_CharacterController.m_ControlConfig.LeftHorizontal, controller.m_CharacterController.m_ControlConfig.LeftVertical);
                }
                return;
