    ...
    public class Pause : _Action
    {
        ...
        private void PauseGame(GMStateController controller)
        {
            if (!GMController.instance.gameEnded && GMController.instance.gameCDEnded) 
            {
                ...
                {
                    if (GMController.instance.gameStart && Input.GetButtonDown(GMController.instance.playerInfo[i].PlayerController.inputMapping.pauseInput))
                    {
                        GMController.instance.gameStart = false;                                        // pause the scripts 

                        Time.timeScale = 0;
                    }                  
                    else if(!GMController.instance.gameStart && Input.GetButtonDown(GMController.instance.playerInfo[i].PlayerController.inputMapping.pauseInput))
                    {
